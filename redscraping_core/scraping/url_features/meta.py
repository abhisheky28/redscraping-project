# redscraping_core/scraping/url_features/meta.py
...
def extract_all_meta(source, is_selenium=False):
    """Extracts ALL <meta> tags (name, property, http-equiv) and their content."""
    meta_data = {}
    
    if is_selenium:
        elements = source.find_elements(By.TAG_NAME, "meta")
        for el in elements:
            # Meta tags identify themselves using one of these three attributes
            key = el.get_attribute("name") or el.get_attribute("property") or el.get_attribute("http-equiv")
            content = el.get_attribute("content")
            if key and content:
                meta_data[key] = content.strip()
    else:
        elements = source.find_all("meta")
        for el in elements:
            key = el.get("name") or el.get("property") or el.get("http-equiv")
...
def run_meta_scraper(input_file, output_format, restart):
    base_dir = os.getcwd()
    urls = []
    ...
    cache_file = os.path.join(cache_dir, f"meta_{input_file.replace('/', '_').replace(':', '_')}.json")
    ...
    engine = UrlEngine()
    try:
        for url in track(urls_to_scrape, description="[cyan]Scraping Meta Tags..."):
            data = engine.fetch(url, extract_all_meta)
            row = {"URL": url, **data}
            results.append(row)
    ...
    out_path = os.path.join(output_dir, f"{base_name}_meta.{'xlsx' if output_format == 'excel' else output_format}")
    
    # Pandas is smart: it will automatically create columns for every unique meta tag it finds!
