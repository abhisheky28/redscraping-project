...
def extract_title(source, is_selenium=False):
    """Extracts the <title> tag from the page."""
    if is_selenium:
        title = source.title
    else:
        title = source.title.string if source.title else "Missing Title"
        
    return {
        "Title": title.strip() if title else "Missing Title",
    }

def run_titles_scraper(input_file, output_format, restart):
    base_dir = os.getcwd()
    urls = []
    ...
    cache_file = os.path.join(cache_dir, f"titles_{input_file.replace('/', '_').replace(':', '_')}.json")
    ...
    engine = UrlEngine()
    try:
        for url in track(urls_to_scrape, description="[cyan]Scraping Titles..."):
            data = engine.fetch(url, extract_title)
            row = {"URL": url, **data}
            results.append(row)
    ...
    out_path = os.path.join(output_dir, f"{base_name}_titles.{'xlsx' if output_format == 'excel' else output_format}")
